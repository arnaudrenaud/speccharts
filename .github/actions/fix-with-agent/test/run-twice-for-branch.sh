...
# passed at all, so it falls back to its "false" default) under real GitHub
# Actions composite-action semantics, using nektos/act
# (https://github.com/nektos/act) instead of a hand-rolled reimplementation
# of GitHub's expression evaluator. Complements
...
# (branch, success-check) and no run-no-more-than-once-ever-for-branch input.
# Runs act against that fixture with --bind (safe here since the fixture is
# disposable — never do this against a real checkout) and asserts, from
# act's own log output, that the second invocation's real steps (in
# particular "Run agent") execute just like the first.
#
...
# per script run keeps things collision-free and the test deterministic
# regardless of what earlier runs left behind.
WORK_BRANCH="no-guard-test-branch-$(date +%s)-$$"
